...
impl AgentService {
    ...
    pub async fn delete_agent(&self, agent_id: &String) {
        let agent: anyhow::Result<Agent> = self._find_agent_by_id(agent_id).await;
    }
    ...
    pub async fn list_agents(&self, symbol: &Option<String>) -> anyhow::Result<()> {
        match symbol {
            ...
            None => {
                let agents = self._get_current_selected_agent().await?;
            }
        }
    ...
    async fn _find_agent_by_id(&self, agent_id: &String) -> anyhow::Result<Agent> {
        ...
        match result {
            Some(some_result) => match some_result.data {
                AgentDataDTO::Single(dto) => Ok(dto.try_into()?),
