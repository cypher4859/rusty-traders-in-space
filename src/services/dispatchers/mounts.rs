...
impl MountService {
    ...
    async fn _list_mounts_by_ship(&self, ship_symbol: &String) -> anyhow::Result<MountDataEnvelopeEnumDTO> {
        ...
        let response = self.st.get_with_headers::<MountDataEnvelopeEnumDTO>(&endpoint, Some(headers), true).await?;
        match response {
            Some(res) => {
                Ok(res)
            }
    ...
    async fn _install_mount_to_ship(&self, ship_symbol: &String, mount_name: &String) -> anyhow::Result<MountInstallDataEnvelopeDTO> {
        ...
        let response = self.st.post_with_headers::<MountInstallDataEnvelopeDTO, RequestMountDTO>(&endpoint, Some(&body), Some(headers), true).await?;
        Ok(response)
    }
    ...
    async fn _remove_mount_from_ship(&self, ship_symbol: &String, mount_name: &String) -> anyhow::Result<MountRemoveDataEnvelopeDTO> {
        ...
        let response = self.st.post_with_headers::<MountRemoveDataEnvelopeDTO, RequestMountDTO>(&endpoint, Some(&body), Some(headers), true).await?;
        Ok(response)
    }
}